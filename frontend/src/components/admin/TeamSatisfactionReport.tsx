...
import { Star, Users, Award, TrendingUp, BarChart3, CheckCircle2, MessageSquare, Zap } from "lucide-react";
...
export function TeamSatisfactionReport() {
  ...
  const totalSurveys = stats.reduce((acc, curr) => acc + curr.totalSurveysCount, 0);
  const activeTeamsWithSurveys = stats.filter((t) => t.totalSurveysCount > 0);
  const globalAverage = activeTeamsWithSurveys.length > 0
    ? (activeTeamsWithSurveys.reduce((acc, curr) => acc + curr.averageRating, 0) / activeTeamsWithSurveys.length).toFixed(1)
    : "0";
  const topTeam = stats.length > 0 && stats[0].totalSurveysCount > 0 ? stats[0] : null;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 dark:border-slate-800 pb-4 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Real-time performance metrics, team benchmarks, and comparative satisfaction ratings.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Across all evaluated teams</p>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold mt-1 flex items-center gap-1">
              ...
              {topTeam ? `${topTeam.averageRating} Avg Score (${topTeam.totalSurveysCount} Surveys)` : "No ratings yet"}
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Total feedback submissions</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <span className="text-[11px] text-slate-400 font-medium">1.0 - 5.0 Scale</span>
            </div>
                  ...
                  <div key={team.teamId} className="space-y-1.5">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                        {team.teamName}
                        {team.totalSurveysCount === 0 && (
                          <span className="text-[10px] text-slate-400 font-normal">(No Data)</span>
                        )}
                      </span>
                      <span className="font-extrabold text-slate-900 dark:text-white flex items-center gap-1">
                        <Star className="h-3 w-3 text-amber-500 fill-amber-500" />
                        {team.totalSurveysCount > 0 ? team.averageRating : "0.0"}
                      </span>
                    </div>
                    ...
                    <div className="h-3 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden relative">
                      <div
                        ...
                        style={{ width: `${team.totalSurveysCount > 0 ? percentage : 0}%` }}
                      />
                    </div>
                  </div>
                );
          ...
          {/* CHART 2: MULTI-METRIC COMPARISON (OVERALL VS COMM VS SOLUTION) */}
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <div className="flex items-center gap-3 text-[10px] font-semibold">
                ...
                <span className="flex items-center gap-1 text-slate-600 dark:text-slate-300">
                  <span className="h-2 w-2 rounded-full bg-emerald-500" /> Solution
                </span>
              </div>
            </div>
                    ...
                    <div className="flex items-center gap-2 text-[10px]">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(team.averageRating / 5) * 100}%` }}
                        />
                      </div>
                    ...
                    <div className="flex items-center gap-2 text-[10px]">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(team.averageCommunicationRating / 5) * 100}%` }}
                        />
                      </div>
                      <span className="w-6 text-right font-bold text-slate-700 dark:text-slate-300">
                    ...
                    <div className="flex items-center gap-2 text-[10px]">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(team.averageSolutionRating / 5) * 100}%` }}
                        />
                      </div>
                      <span className="w-6 text-right font-bold text-slate-700 dark:text-slate-300">
